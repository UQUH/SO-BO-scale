function plot_sota_bo_ex1(n_mc_eval, acquisition_function)
%% Plot SOTA noisy-BO baseline results for Ex1
...
data = load(fullfile(results_dir, sprintf('bo_ex1_%s.mat', results_tag)));
...
filename = sprintf('f_sota_%s_Ex1.pdf', results_tag);
...
filename = sprintf('hist_beta_sota_%s_Ex1.pdf', results_tag);
exportgraphics(gcf, fullfile(results_dir, filename), 'ContentType', 'vector');
fprintf('Saved: %s\n', filename);
