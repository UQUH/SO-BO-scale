#!/usr/bin/env python3
"""
SOTA noisy-BO baseline for Ex1 using qNEI/qLogNEI without explicit replicates.
...
# GP hyperparameters
lengthscale_init = 0.8
lengthscale_bounds = (0.05, 1.0)
noise_init = 0.85
noise_bounds = (1e-4, 2.0)
...
beta_grid_np = np.arange(beta_min, beta_max + 1, dtype=float)

print(f"Loaded dist matrix: {Nrep} repetitions x {num_beta} beta values")
print(f"Beta range: [{beta_min}, {beta_max}]")
print(f"Reference distance (doExp): {do_exp:.10e}")


def get_dist_for_beta(beta, rep_idx):
    col_idx = int(np.clip(beta - k, 0, num_beta - 1))
...
print("\n" + "=" * 60)
print("SOTA NOISY BO BASELINE (Ex1)")
print("=" * 60)
