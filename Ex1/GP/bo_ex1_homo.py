...
stagnation_tol = 1e-14  # Improvement tolerance for early stopping
# Use homoscedastic noise with log transform (hetero doesn't make sense in log space)
noise_model = "homo"
noise_floor = 1e-8      # Minimum observation noise variance
...
lengthscale_init = 0.3            # Reasonable for normalized [0,1] input
...
def evaluate_beta(beta, eval_seed=None):
    ...
    Returns MSE = (dosrom - doExp)^2 for a single MC sample drawn from dist_matrix.
    ...
    -------
    mean_mse : float
        MSE value (single sample, so just (dosrom - doExp)^2)
    mean_dosrom : float
        L2 distance value
    mse_var : float
    ...
    # Get the pre-computed L2 distance
    dosrom = get_dist_for_beta(beta, rep_idx)

    # Compute MSE
    mse = (dosrom - doExp) ** 2

    return float(mse), float(dosrom), 0.0
...
def plot_gp_iteration(model, train_x, train_y, bounds, grid, iteration,
    ...
    with torch.no_grad():
        ...
        # Draw a sample from posterior (Thompson sample)
        ts_sample = posterior.rsample().squeeze().numpy()
    ...
    # --- Left plot: GP posterior mean and confidence interval ---
    ax1 = axes[0]
    ax1.fill_between(grid_np, mean - 2*std, mean + 2*std,
    ...
    ax1.set_title(f'Iteration {iteration}: GP Posterior', fontsize=14)
...
def thompson_sampling(model, grid, batch_size=1):
    """
    ...
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        ...
        sample = posterior.rsample().squeeze()

        if debug_selection:
            print("  TS: selecting top-k samples...", flush=True)
        k_top = min(batch_size * 3, sample.numel())
        _, indices = torch.topk(sample, k_top, largest=False)
        if debug_selection:
            print(f"  TS: top-k done (k={k_top}).", flush=True)

    # Select top candidates (smallest sampled values)
    candidates = []
    for idx in indices[:batch_size * 3]:  # Check more in case of duplicates
...
def select_next_candidate(model, bounds, grid, best_y, train_x, batch_size=1):
    ...
    """
    if acquisition_function == "thompson":
        return thompson_sampling(model, grid, batch_size)
    if acquisition_function == "qlognei":
        sampler = SobolQMCNormalSampler(torch.Size([128]))
        objective = GenericMCObjective(lambda Y, X: -Y.squeeze(-1))
        acq = qLogNoisyExpectedImprovement(
...
for kernel_type in kernel_list:
    for acquisition_function in acquisition_functions:
        ...
        out_path = results_dir / "bo_ex1.mat"
