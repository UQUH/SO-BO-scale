...
noise_model = "hetero"
n_mc_var = 1           # Number of MC samples to estimate variance at each beta
...
def evaluate_beta(beta, eval_seed=None):
    ...
    ddof = 1 if len(mse_samples) > 1 else 0
    mse_var = np.var(mse_samples, ddof=ddof)  # Sample variance
...
base_results_dir = Path(custom_results_dir) if custom_results_dir else script_dir / "Results" / "mc1"
...
for kernel_type in kernel_list:
    for acquisition_function in acquisition_functions:
        ...
        out_path = results_dir / "bo_ex1_mc1.mat"
