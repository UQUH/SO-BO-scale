...
% Ex3: Nonlinear log-log scale with homoscedastic noise
...
load('dist.mat', 'doExp');  % Load target value from dist.mat

%% Optimization for hyperparameter selection
% Start optimization timer
tic;

% Define search interval for beta (Ex3: 1 to 100)
boundBeta = [min(DTstat.beta), max(DTstat.beta)];
...
% Get floor and ceil of each element (these are the beta values evaluated per iteration)
beta_floor = floor(store_beta_values);
beta_ceil  = ceil(store_beta_values);

% Interleave floor and ceil to preserve iteration order
% Each iteration evaluates floor(beta) and ceil(beta)
nIter = length(store_beta_values);
allBeta = zeros(2*nIter, 1);
allBeta(1:2:end) = beta_floor;
allBeta(2:2:end) = beta_ceil;

% Keep unique values but preserve order (for iteration plotting)
[uniqueBeta, ia] = unique(allBeta, 'stable');

% Number of unique betas
N = length(uniqueBeta);

% Create iteration labels based on when each beta was first evaluated
iterLabels = ceil(ia / 2);  % Which iteration each unique beta came from

%% Plot 1: Beta Evolution Across Iterations
figure('Units', 'inches', 'Position', [0, 0, width_plot, height_plot]);
scatter(iterLabels, uniqueBeta, 'k', 'filled'); % Scatter plot of beta values by iteration
hold on

% Add rectangle for optimal region
nIterTotal = max(iterLabels);
xFill = [0.5, nIterTotal+0.5, nIterTotal+0.5, 0.5]; % X-coordinates of rectangle
...
xlim([0, nIterTotal+1]); % Set x-axis limits
xticks(1:1:nIterTotal); % Set x-axis tick marks for each iteration
xlabel('Iteration number'); % x-axis label
ylabel('$\beta$'); % y-axis label

legend('$\beta$ sample', 'Optimal region', 'Location', 'northeast', 'Box', 'off');

filename = 'series_betaOpt_1D_Ex3.pdf';
...
% Sort uniqueBeta for plotting objective function
sortedBeta = sort(uniqueBeta);

% Extract the MSE values that correspond to those betas
mse_values = DTstat.mse(ismember(DTstat.beta, sortedBeta))/mseLocalMin;

load("dist.mat")
% Get indices for sortedBeta in DTstat
[~, betaIdx] = ismember(sortedBeta, DTstat.beta);
mse_mat = (dist(:, betaIdx) - doExp).^2;
...
groupData = repelem(sortedBeta, 1000)';
h = boxchart(groupData,mse_mat_vec,'MarkerColor',greenshade2','BoxFaceColor',greenshade2, 'BoxFaceAlpha', 0.5);
p1 = plot(sortedBeta,mse_values,'-o','color',blue, 'MarkerSize', 4,'LineWidth',th); %iteration wise beta opt
...
xlim([min(sortedBeta)-5, max(sortedBeta)+5])
xlabel('$\beta$');
ylabel('$f / f^*$');
...
legend([h, p1, p2, p3, p4], {'f() via MC samples', ...
    ...
    'Position', [0.6,0.65,0.15,0.15], 'Box', 'off'); % Legend positioning
hold off; % Release hold on plot

filename = 'Obj_fun_eval_1D_Ex3.pdf';
output_path = fullfile(pwd, 'Results', filename);% Combine into a full relative path
exportgraphics(gcf, output_path, 'ContentType', 'vector');% Export the figure