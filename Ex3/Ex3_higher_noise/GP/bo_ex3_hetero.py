#!/usr/bin/env python3
"""
Bayesian Optimization for Ex3: Nonlinear log-log scale.
...
n_initial = env_int("GP_N_INITIAL", 10)          # Number of initial samples
n_iterations = env_int("GP_N_ITERATIONS", 50)       # Number of BO iterations
batch_size = env_int("GP_BATCH_SIZE", 10)         # Number of candidates per iteration
ts_num_samples = env_int("GP_TS_NUM_SAMPLES", 100)    # Number of posterior samples for batch Thompson
grid_size = env_int("GP_GRID_SIZE", 200)         # Grid size for Thompson Sampling
# Posterior beta* variance stopping criterion
posterior_var_tol = 0.7      # Stop when variance drops below this threshold
posterior_var_streak = 3     # Stop after this many consecutive hits
posterior_var_samples = env_int("GP_POST_VAR_SAMPLES", 100)  # Posterior samples to estimate beta* variance
noise_model = "hetero"
n_mc_var = env_int("GP_N_MC_VAR", 10)           # Number of MC samples to estimate variance at each beta
noise_floor = env_float("GP_NOISE_FLOOR", 0.01)      # Minimum noise variance in LOG space
...
# Use log-transform for MSE (helps with numerical stability)
use_log_transform = True

# RBF kernel for log-noisy EI
kernel_type = os.environ.get("GP_KERNEL_TYPE", "rbf")
...
lengthscale_init = env_float("GP_LENGTHSCALE_INIT", 0.3)            # Reasonable for normalized [0,1] input
lengthscale_bounds = (
    env_float("GP_LENGTHSCALE_MIN", 0.05),
    env_float("GP_LENGTHSCALE_MAX", 1.0),
)  # Allow learning
...
noise_init = env_float("GP_NOISE_INIT", 0.1)                    # Initial noise with more MC samples
noise_bounds = (
    env_float("GP_NOISE_MIN", 1e-4),
    env_float("GP_NOISE_MAX", 5.0),
)        # Tighter bounds with averaged observations
...
acquisition_functions = [os.environ.get("GP_ACQUISITION", "thompson")]
...
def evaluate_beta(beta, eval_seed=None):
    ...
    mse_var : float
        Variance of MSE (for heteroscedastic GP), transformed to log space if use_log_transform
    """
    global eval_count
    eval_count += 1
    beta = int(round(beta))
    ...
    if use_log_transform and mean_mse > 0:
        ...
        return float(mean_mse), float(mean_s), float(log_var)
    else:
        return float(mean_mse), float(mean_s), float(max(mse_var, noise_floor))
...
def plot_gp_iteration(model, train_x, train_y, bounds, grid, iteration,
    ...
    if use_log_transform:
        ax1.set_ylabel('log(MSE)', fontsize=12)
    else:
    ...
    if use_log_transform:
        ax2.set_ylabel('log(MSE)', fontsize=12)
    else:
...
def build_gp_model(train_x, train_y, train_yvar=None):
    ...
    # Build model with input normalization for numerical stability
...
base_results_dir = Path(
    custom_results_dir) if custom_results_dir else script_dir / "Results"
...
for kernel_type in kernel_list:
    for acquisition_function in acquisition_functions:
        ...
        for i, beta in enumerate(init_betas):
            ...
            yvar = max(mse_var / max(n_mc_var, 1), noise_floor)
            train_yvar_list.append([yvar])
        ...
        for it in range(1, n_iterations + 1):
            ...
            if use_log_transform:
                train_y = torch.log(train_y.clamp(min=1e-20))
        ...
        if use_log_transform:
            mse_opt_gp = float(np.exp(mean[gp_min_idx]))
        else:
