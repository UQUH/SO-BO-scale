%% Hyper-parameter training via 1-D optimization in conjunction with MC
rng(6) % Set random seed for reproducibility
...
% This section optimizes a hyperparameter (beta)
% using Monte Carlo samples and 1D scalar optimization (fminbnd).
...
% Initialize beta evaluations tracker (can be filled later in fMC)
betaEval = [];

% Define bounds for beta
boundBeta = [k, 100];

% Set optimization options (tolerance for convergence)
options = optimset('TolX', 0.1);

% Perform bounded scalar optimization using fminbnd
[beta_eval, fval] = fminbnd(@(beta) fMC(beta, DTstat), boundBeta(1), boundBeta(2), options);
...
filename = 'series_betaOpt_1D_Ex2.pdf';
...
filename = 'Obj_fun_eval_1D_Ex2.pdf';
output_path = fullfile(pwd, 'Results', filename);% Combine into a full relative path
exportgraphics(gcf, output_path, 'ContentType', 'vector');% Export the figure