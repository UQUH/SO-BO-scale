%% Ablation study for Ex2 BO: initial design bounds (log-uniform sampling)
...
%% Load data
load("dist.mat"); % dist matrix (Nrep x num_Beta)
load("DTstat_table.mat", "DTstat"); % summary table

k = 10;        % ROM dimension (minimum beta)
doExp = 16.1938; % reference distance

%% Build full DT table once
[Nrep, num_Beta] = size(dist);
boundBetaFull = [k, k + num_Beta - 1];
beta_range_full = boundBetaFull(1):1:boundBetaFull(2);
rep_range = 1:1:Nrep;

beta_mat = repmat(beta_range_full, Nrep, 1);
...
boundBeta = [k, 100];
beta_grid = boundBeta(1):1:boundBeta(2);
