%% Bayesian optimization under uncertainty Linear Dynamics problem
...
k = 10; % ROM (Reduced Order Model) dimension
doExp = 16.1938; % reference distance
...
%% plot 0: 
load('dist.mat', 'dist'); % Load distance data between HDM and ROM from file
...
sub_sampled = randperm(1000,10);
logbeta_col = reshape(log(beta_mat(sub_sampled,:)),10*num_Beta,1);
logdosrom_col = reshape(log(dist(sub_sampled,:)),10*num_Beta,1);

q25Logdosrom_col = quantile(log(dist),0.25); % 25th quantile
q75Logdosrom_col = quantile(log(dist),0.75);
% Create figure
figure('Units', 'inches', 'Position', [0, 0, width_plot, height_plot]); 
% Plot raw data (highly noisy with downward trend)
p1 = scatter(logbeta_col, logdosrom_col,'k');
hold on;
% Add local regression function
p2 = plot(logbeta, DTbest.logdosromLocal, 'r', 'LineWidth', 2); % Local regression in red
% Add pointwise quantiles: 25% and 75%
p3 = plot(logbeta, q25Logdosrom_col, 'c', 'LineWidth', 2); % 25th quantile in cyan
p4 = plot(logbeta, q75Logdosrom_col, 'c', 'LineWidth', 2); % 75th quantile in cyan
% Add Bayesian GLM prediction
p5 = plot(DTm1.logbeta, DTm1.ypred, 'b', 'LineWidth', 2); % GLM prediction in blue
% Customize axes
xlabel('ln$\beta$');
ylabel('ln(d$_o$(u$_L$))', 'Rotation', 90); % Rotated ylabel
xlim([2 4.6])
ylim([0.5 4])

% Add legend
legend([p1,p2,p3,p5], {sprintf('Data points (N = %d)', 91000), ...
...
scatter(DT0.logbeta, DT0.logdosrom,sz, 'k', 'filled'); % BO data points
hold on;
...
qt975 = 1.96; % 95% quantile for normal distribution

% Plot the regression function (GLM)
p1 = plot(logbeta, ypred,'color', blue, 'LineWidth', 2);
P2 = fill([logbeta; flipud(logbeta)], ...
     [ypred + qt975 * ysd; flipud(ypred - qt975 * ysd)], ...
     blueshade, 'EdgeColor', 'none', 'FaceAlpha', 0.7);
% Plot the local regression function
plot(DTbest.logbeta, DTbest.logdosromLocal, 'r', 'LineWidth', 2);
% Plot the generalized linear model using all data
plot(DTm1.logbeta, DTm1.ypred, 'b', 'LineWidth', 2);
% Add a legend
legend(['BO data points: N = ', num2str(size(DT0, 1))], ...
...
ylabel('$f / f^*$');
% 
% Define scale factor using the minimum MSE value
...
fill(xVertices, yVertices, [0.5, 0.5, 0.5], 'FaceAlpha', 0.1, 'EdgeColor', 'none'); % Gray color, 10% opacity
...
p6 = plot(DToptLm.betaOpt(DToptLm.iter == iterplot), DToptLm.fOpt(DToptLm.iter == iterplot) / mseScale, ...
     'x', 'Color',blue, 'MarkerSize', 10, 'LineWidth', 2);

% Plot the best estimate of objective function and optimum
...
% Plot vertical line at betaOptLocal
xline(betaOptLocal, '--', 'LineWidth', 1);

% Plot objective function and optimum using all data
...
% Add legend
lgd = legend([p1,h1,p3,p4,p5,p6,p7,p8],{'f() via GLM: N = ' + string(height(DT0)), ...
...
%lgd.Position = [0.65, 0.65, 0.1, 0.1]; % Adjust as needed
% Final adjustments
hold off;

filename = 'f_glm_TS_Ex2.pdf';
output_path = fullfile(pwd, 'Results', filename);% Combine into a full relative path
exportgraphics(gcf, output_path, 'ContentType', 'vector');% Export the figure
...
filename = 'hist_betaOpt_Ex2.pdf';
output_path = fullfile(pwd, 'Results', filename);% Combine into a full relative path
exportgraphics(gcf, output_path, 'ContentType', 'vector');% Export the figure

%% plot 4: series sample
iter = [zeros(nInitial, 1); repelem((1:BO_iter)', nBatch)]; % Iterations array

% Create a table equivalent to DT0
DT_plot3 = table('Size',[length(iter),2],'VariableTypes',{'double','double'},'VariableNames',{'Iter','Beta'});
DT_plot3.Iter = iter;
DT_plot3.Beta = DTid.beta;
% Create the plot
figure('Units', 'inches', 'Position', [0, 0, width_plot, height_plot]); 
hold on;
% Set up axes and labels
xlim([-1, BO_iter+1]); % Adjust based on range
ylim([k-3, 100.1]); % Adjust for better visibility
% Add points
scatter(DT_plot3.Iter, DT_plot3.Beta, 'k');
% Add rectangle for optimal region using the fill function
xFill = [-1, BO_iter, BO_iter, -1]; % X-coordinates of the rectangle
yFill = [betaOptRange(1),  betaOptRange(1),betaOptRange(end), betaOptRange(end)]; % Y-coordinates of the rectangle
% Draw the filled rectangle
fill(xFill, yFill, [0, 0, 0], 'FaceAlpha', 0.1, 'EdgeColor', 'none');
% Add legend
legend('$\beta$ sample', 'Optimal region', ...
    'Location', 'northeast', 'Box', 'off');
% Annotate axes
xticks(0:5:BO_iter)
...
filename = 'series_sample_Ex2.pdf';
output_path = fullfile(pwd, 'Results', filename);% Combine into a full relative path
exportgraphics(gcf, output_path, 'ContentType', 'vector');% Export the figure
...
hold on;
p1 = plot(DToptLm.iter,DToptLm.betaOpt,'-o','color',blue, 'MarkerSize', 4,'LineWidth',th); %iteration wise beta opt
...
% Add rectangle for optimal region using the fill function
xFill = [-0.5, BO_iter+1, BO_iter+1, -0.5]; % X-coordinates of the rectangle
yFill = [betaOptRange(1),  betaOptRange(1),betaOptRange(end), betaOptRange(end)]; % Y-coordinates of the rectangle
% Draw the filled rectangle
fill(xFill, yFill, [0, 0, 0], 'FaceAlpha', 0.1, 'EdgeColor', 'none');
% Customize axes
ax = gca; % Get current axes
ax.XTick = 0:5:BO_iter+1; % Change tick positions
ax.XTickLabel = 0:5:BO_iter+1; % Change labels
xlabel('BO iteration'); % X-label
ylabel('$\beta^*$'); % Y-label
xlim([-0.5 BO_iter+1])
ylim([k+6 60])
% Add legend
...
filename = 'series_betaOpt_Ex2.pdf';
output_path = fullfile(pwd, 'Results', filename);% Combine into a full relative path
<<<<<<< HEAD
exportgraphics(gcf, output_path, 'ContentType', 'vector');% Export the figure
...
save(fullfile(pwd, 'Results', 'bo_stats_Ex2.mat'), '-struct', 'bo_stats');
=======
exportgraphics(gcf, output_path, 'ContentType', 'vector');% Export the figure
>>>>>>> b3229bacb6f237d710e56b960cef1e890be6f428
