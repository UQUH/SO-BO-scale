...
def postprocess_sota_qlognei_seeded(example: str, seed: int) -> None:
    raw_dir = raw_results_dir(example, "SOTA_BO", seed)
    key = EXAMPLE_SCRIPT_KEY.get(example, example.lower())
    raw_mat = raw_dir / f"bo_{key}_qlognei_mc10.mat"
    if not raw_mat.exists():
        raise FileNotFoundError(f"Missing native SOTA output: {raw_mat}")
    ...
    print(f"Saved SOTA robustness result: {out_path}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Postprocess one seeded qLogNEI robustness job.")
    parser.add_argument("--example", required=True, choices=["Ex1", "Ex2", "Ex3", "Ex3_sigma03"])
    parser.add_argument("--seed", required=True, type=int)
    parser.add_argument("--skip-if-exists", action="store_true")
    args = parser.parse_args()
    out_path = seed_result_path(args.example, "SOTA_BO", args.seed)
    if args.skip_if_exists and out_path.exists():
        print(f"Skipping existing SOTA robustness result: {out_path}")
        return
    postprocess_sota_qlognei_seeded(args.example, args.seed)
